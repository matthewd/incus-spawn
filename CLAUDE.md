...
**RubyGems payload caching:** `rubygems.org` and `index.rubygems.org` are fixed built-in routes. Only complete, bounded Compact Index `/info/<gem>` responses observed over TLS in the current proxy process may authorize a payload cache read for ten minutes; observations are origin- and generation-scoped, while metadata remains uncached and retains Range, conditional, ETag, and `Repr-Digest` behavior. Only unconditional, bodyless, unauthenticated, query-free `GET /gems/*.gem` requests use the cache. Payloads are bounded, single-flight by digest, SHA256-verified before serving, atomically published under `~/.cache/incus-spawn/rubygems/objects/sha256/`, and re-hashed once per process before reuse. Range, conditional, authenticated, redirected, checksum-free, or otherwise ambiguous traffic must relay unchanged rather than broadening cache semantics.
